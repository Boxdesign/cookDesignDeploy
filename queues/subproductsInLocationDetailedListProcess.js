...
queue.process('subproductsInLocationDetailed', function(job, done){
  ...
  async.waterfall([
    ...
    (cb) => {
        ...
        printBook.subproductsInLocationDetailed(Model, userLocIds, userProfile, templateId, tax, filterLocation, (err, stream) => {
	      ...
	      var s3obj = new AWS.S3({
	        params: {
	          ...
	          Key: 'reports/subpsInLocationDetailed-'+moment().format('YYYYMMDDHHmm')+'.pdf',
						...
						{	url: config.apiUrl + '/socketio/send', 
							...
							body: 
								{	
					 			...
					 			logger.info("Report of detailed subproducts in location completed successfully. File saved in ", url)
