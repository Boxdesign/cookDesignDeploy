...
//let schedule = process.env.NODE_ENV == 'production' ? '0 22 * * 1-4' : '0 4 * * *' //Prod at 22:00 UTC / 0:00 CET and dev evert 5 min
var loggerHelper = require('../helpers/logger');
const logger = loggerHelper.selentaDishWebService;
var schedule;

switch(process.env.NODE_ENV) {
	case 'production':
			schedule = '0 22 * * 1-4'  //Prod at 22:00 UTC / 0:00 CET
	break;

	case 'staging':
			schedule = '0 22 * 12 1-4'
	break;
...
exports.dishTask = cron.schedule(schedule, () => {
  ...
  var costHelper= require ('../helpers/cost');
	...
	logger.debug('Dish Web Service - Starting job...');
	...
	async.waterfall([
		...
		(cb) => {
			...
			Location.findOne(
					{"referenceNumber": "D600"} 	//Reference number of Hotel Sofia S.L. location
				)  
				.exec((err,doc)=> {
					...
					logger.debug('Found Hotel Sofia location for the dish web service!');
			...
			WebServiceLog.find(
				...
				.exec((err,logs) => {
			 		...
			 		if(logs && logs.length){ 
			 			...
			 			lastExportDate = logs[0].date //Found last successful log for dish web service
						logger.debug('Found last successful web service log for the dish web service! %s', lastExportDate);
  	      ...
  	      {$match: {'location': {$in: locationFilter}}},	
   				{$match:{"versions.updatedAt": {$gte: lastExportDate}}}
					...
					Dish.populate(doc,{path: "measurementUnit family family.subfamilies versions.gallery location"}, (err, docs) => {
              if (err) return cb(err)
              
              dishesToExport = docs;

              logger.debug('There are %s dishes to be exported.', doc.length);
              ...
              cb(null, doc)
				...
				async.eachSeries(dishesToExport, (dish, cb_async1 )=>{

					if(dish.versions.composition.length){

							async.eachSeries(dish.versions.composition, function(compElement,cb_async2){
					 ...
					 cb(null, doc)
						...
						MeasurementUnit.populate(dishesToExport,{path:"versions.composition.measuringUnit"},(err,docs)=>{
							if(err) return cb(err)
							cb(null, doc)
						})
						...
						Process.populate(dishesToExport,{path:"versions.cookingSteps.process"},(err,docs)=>{
							if(err) return cb(err)
							cb(null, doc)
						})
						...
						Utensil.populate(dishesToExport,{path:"versions.cookingSteps.utensil"},(err,docs)=>{
							if(err) return cb(err)
							cb(null, doc)
						})
						...
						Family.populate(dishesToExport,{path:"versions.cookingSteps.utensil.externalFamily"},(err,docs)=>{
							if(err) return cb(err)
							cb(null, doc)
						})	
						...
						CheckPoint.populate(dishesToExport,{path:"versions.cookingSteps.criticalCheckpoint versions.cookingSteps.gastroCheckpoint"}, (err,docs)=>{
							if(err) return cb(err)
							cb(null, doc)
						})
				...
				costHelper.calculateAvgRecipeLocCostAndAllergens(dishesToExport, Dish);
				cb(null, doc)						
					...
					dishesToExport.forEach((dish)=>{
						...
						if(dish.versions.lang && dish.versions.lang.length) name = dish.versions.lang[0].name; else name='<NO DISPONIBLE>';
						if(dish.versions.numServings) numServings = dish.versions.numServings; else numServings = 0;
						if(dish.versions.weightPerServing) weightPerServing = dish.versions.weightPerServing; else weightPerServing = 0;
						if(dish.versions.gallery && dish.versions.gallery.sizes && dish.versions.gallery.sizes.length ) image = dish.versions.gallery.sizes[1].url; else image='<NO DISPONIBLE>'
						if(dish.versions.locationCost && dish.versions.locationCost.length) cost = dish.versions.locationCost[0].unitCost; else cost = dish.versions.unitCost;
						if(dish.measurementUnit && dish.measurementUnit.referenceCode) measuringUnitCode = dish.measurementUnit.referenceCode; else measuringUnitCode='<NO DISPONIBLE>';
						if(dish.family && dish.family.externalCode) {family = dishCode + dish.family.externalCode.toString()	} else {	family = dishCode + noFamilyCode;}

						costPerServing = dish.versions.costPerServing;

						if(dish.caducityFresh && dish.caducityFresh.value != 0) {
							if(dish.caducityFresh.timeUnit == 'd.')	caducityFresh = dish.caducityFresh.value;
							else caducityFresh = dish.caducityFresh.value * 30;
						} else {
							caducityFresh = 0;
						}

						if(dish.caducityFreeze && dish.caducityFreeze.value != 0) {
							if(dish.caducityFreeze.timeUnit == 'd.') caducityFreeze = dish.caducityFreeze.value;
							else caducityFreeze = dish.caducityFreeze.value * 30;
						} else {
							caducityFreeze = 0;
						}

						if(dish.daysToUse && dish.daysToUse.value != 0) {
							if(dish.daysToUse.timeUnit == 'd.')	daysToUse = dish.daysToUse.value;
							else daysToUse = dish.daysToUse.value * 30;
						} else {
							daysToUse = 0;
						}
						
						if(dish.versions.batchWeight) optimumWeight=dish.versions.batchWeight; 
						else if(dish.versions.netWeight) optimumWeight=dish.versions.netWeight; 
						else optimumWeight=0;

						if(dish.subfamily){	

							subfamilyId = new ObjectId(dish.subfamily) 

							dish.family.subfamilies.forEach((subfam)=>{
								let subId = new ObjectId(subfam._id)							

								if(subfamilyId.equals(subId)){
									let subfamExternalCode = subfam.externalCode || '99001000'
									subfamily = dishCode + subfamExternalCode.toString();
								} 								
							})
						...
						if(dish.versions.composition.length){
							...
							dish.versions.composition.forEach((compositionElement,index)=>{

								let compElementName;
								let compElementGrossWeight;
								let compElementMeasUnit;
								let referenceNumber;

								compElementName = compositionElement.name;
								if(compositionElement.grossWeight) compElementGrossWeight = compositionElement.grossWeight.toFixed(3); else compElementGrossWeight=0;
								if(compositionElement.measuringUnit && compositionElement.measuringUnit.referenceCode) compElementMeasUnit = compositionElement.measuringUnit.referenceCode; else compElementMeasUnit='<N/A>'
								if(compositionElement.element.item && compositionElement.element.item.referenceNumber) referenceNumber = compositionElement.element.item.referenceNumber; else referenceNumber = '000000000000000000000';

								 compositionObj = {
								 	"COMP_REFERENCIA" : referenceNumber,
								 	"NOMBRE" : compElementName,  //For dishes, we have removed the versions key and added the active version in the previous step
						...
						if(dish.versions.cookingSteps.length){

							dish.versions.cookingSteps.forEach((cookingStep, index)=>{
								...
								if(cookingStep.gastroCheckpoint) checkpoint = cookingStep.gastroCheckpoint.lang[0].name; else checkpoint='';
  					...
  					/* ----------------  Generate dish object -----------------*/
					  dishObj = {
							"CENTRO" : locationHotelSofia.referenceNumber.substring(0, 4),
							"NOMBRE_PLATO": name.substring(0, 40).toUpperCase(),
							"REFERENCIA": dish.referenceNumber,
							"TIPO_MATERIAL": "ZINV",
							"ESTADO": "",
							...
							"PESO_RACION": weightPerServing,
							"COSTE_RACION": costPerServing,
							"COMPOSICION": compositionTable,
							"ELABORACION": cookingStepsTable,
							"VAJILLA": '',
							"PPVV": []
						}
						dishes.DATA.push(dishObj)
						logger.debug('DishObj : %j', dishObj)

					})
					
					logger.debug('Generated array of dish objects to export with %s objects.', dishes.DATA.length);
					cb(null,doc)
					...
					async.during(
					    (callback) => { //asynchronous truth test to perform before each execution of fn. Invoked with (callback).
					        return callback(null, page * perPage < dishes.DATA.length);
					    },
					    (callback) => { //An async function which is called each time test passes. Invoked with (callback).
					        let dataPage = { "DATA": [] }
					        dataPage.DATA = dishes.DATA.slice(page * perPage, (page + 1) * perPage); //end position is not included in slice. Slice extracts up to but not including end.
					        page++;

									if(dataPage.DATA.length) {

										request.post({url: config.selenta.wsDishUrl, json: true, body: dataPage}, (err, res, body) => {
			...
			if(dishes.DATA.length) {
						...
						let logEntry = {
							type : 'dish',
							elementsToExport: dishes.DATA.length,
						...
						log.save((err, doc) => {
							if(err) return cb(err)
		 					logger.debug('Saved log entry in WebServiceLog collection. %j', logEntry);
